...
// Version 1.1 15-12-2025
// *********************
// 1.1 What's new: The 3 swap functions came in handy in my situation, hope it helps you too
// *********************
// -- Hardware --
// Display = OLED 0.91
...
// MPU6050 sensor
// -- Connections --
// SDA = D2 // SCK or SCL = D1 // Internal Led = D4
...
// New: Adafruit_Sensor *mpu_temp, *mpu_accel, *mpu_gyro; //new
...
float xas, yas, zas, gxas, gyas, gzas, swap; // Levels in floating integer (swap = variable used in swapping process)
float xas_raw[100], yas_raw[100]; // raw values in array so we can average over 100 values
...
void loop() {
  ...
  // Get 30 values and use the average
  ...
  yas = 0;
    for (int i = 0; i <= 49; i++) { // Get 50 values of x and y and taken the average
      mpu_accel->getEvent(&accel); //New
      xas_raw[i]= accel.acceleration.x;
      xas = xas + xas_raw[i];
      yas_raw[i]= accel.acceleration.y;
      yas = yas + yas_raw[i];
      delay(5); // 5ms * 50 = 250 ms
      }
  
  // We've gathered 30 values, dividing the sum by 30 will give us the average
  ...
  // Enqoute the strings for X and Y ax for JSON, we do this _after_ displaying them on Oled
  xas$ = "\"" + xas$ + "\"";
  yas$ = "\"" + yas$ + "\"";
  ...
  //delay(100); // 100ms from the average loop + these 100 ms makes 200 ms = approx in ½sync with the java page refresh for the website
}
